...
import { createKitchenStaff, editKitchenStaff } from "../../api/ManagerRequest";
...
function CreateKitchenStaff({ id, edit, kitchenStaffData }) {
  ...
  const onFinish = async (details) => {
    ...
    try {
      ...
      const { data } = edit
        ? await editKitchenStaff(kitchenStaffData?._id, details)
        : await createKitchenStaff(id, details);

      if (data.success) {
        hideLoader();
        toast.success("New Kitchen Staff Created", {
          position: "top-right",
          autoClose: 3000,
          theme: "dark",
        });
      ...
      <div className="md:w-7/12 flex justify-center overflow-auto scrollbar-hide">
        <div className="w-100">
          <div className="w-12/12  mt-10 ">
            <Title level={2}>
              {edit ? "Edit Kitchen Staff" : "Create Kitchen Staff"}
            </Title>
            ...
            <div className="my-5 flex flex-col gap-2 ">
              <Form
                ...
                onFinish={onFinish}
                initialValues={{
                  name: kitchenStaffData?.name,
                  email: kitchenStaffData?.email,
                }}
                style={{
                  // maxWidth: 600,
                  width: 600,
...
CreateKitchenStaff.propTypes = {
  edit: PropTypes.bool,
  id: PropTypes.string,
  kitchenStaffData: PropTypes.shape({
